...
class Crb_Main_Menu_Walker_Mobile extends Walker_Nav_Menu {
    ...
    public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
        ...
        foreach ( $atts as $attr => $value ) {
            ...
            if ( ! empty( $value ) ) {
                ...
                if ( 'href' === $attr ) {
                    ...
                    $value = ( ! $zip_code && $show_modal ) ? esc_attr( $value . ' btn-modal mobile-nav__link' ) : esc_attr( $value );
                } else {
        ...
        $item_output .= '<a'. $attributes .' class="mobile-nav__link" >';
        $item_output .= $args->link_before . apply_filters( 'the_title', $item->title, $item->ID ) . $args->link_after;
        $item_output .= '</a>';
        if ($args->walker->has_children && $depth === 0) {
            $item_output .= '<button class="mobile-nav__trigger" type="button">
