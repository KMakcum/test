jQuery(document).ready(function ($) {
    ...
    $('body').on('click', 'a.meals-mpw', function (e) {
        if (mealsLeft === 0 && parseInt(countFilledMeals) === 14) {
            $('.btn-modal[href="#js-modal-remove-item"]').click()
            return false
        }
        ...
        $('#meal-plan-modal').css({'display': 'flex', 'visibility': 'unset'})
        clearInterval(intervalIDfadeIn)
        intervalIDfadeIn = setTimeout(function () {
            if ($('.meal-plan__body.body-meal-plan').css('display') !== 'block') {
                $('#meal-plan-modal').fadeOut()
            }
        }, 3000)
