(function ($, window, document) {
	...
	var call_ajax = function () {
		...
		let url = target_url.split('/');
		url = url.filter(function( item ) {
			return item !== '';
		});

		let page = url.slice(-1)[0];
		...
		$.ajax({
			...
			success: function (response) {
				...
				let product_unit = $( scroll_options.scroll_item_selector );
				let next = $( scroll_options.scroll_next_selector );
				...
				$(scroll_options.scroll_content_selector).append(response);

				$('.scroll-loader').remove()

				// $(response).fadeTo(2000, 1, function () {
				// 	under_loading = false
				// })
    ...
    //
		// 		$('.scroll-loader').remove()
    //
		// 		product_unit.fadeTo(2000, 1, function () {
		// 			under_loading = false
		// 		})
    //
		// 		$('.products').trigger('ajax_product_load_end')
	...
	$(window).on('scroll touchstart', function () {
		...
		if (!under_loading && !loading_finished && y.scrollTop() >= Math.abs(((off_set.top) / 2) - (y.height() - 150))) {
			call_ajax()
		}
	})
